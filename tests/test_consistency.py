...
from scripts.repo_tools.consistency_scan import ConsistencyScan, format_consistency_report
...
def test_main_reports_an_unexpected_scan_failure(
    ...
    """An unexpected scanner failure should become a concise CLI error."""

    def fail_scan() -> ConsistencyScan:
        raise RuntimeError("fixture scanner failure")

    monkeypatch.setattr(consistency_scan, "scan_consistency", fail_scan)

    assert consistency_cli.main([]) == 1
    assert capsys.readouterr().out == "[consistency] fixture scanner failure\n"
