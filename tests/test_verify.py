...
def test_ci_plan_runs_both_policy_checkers() -> None:
    ...
    assumed: ``platform-check`` 0.6.24 expands ``npm`` and ``.sh`` bodies but
    ...
    rather than an allowlist, so an arbitrary benign command still passes.
    Dropping either checker is a real loss of coverage; see the "Two checkers"
    note in ``scripts/repo_tools/hosted_ci_policy.py``.
    """
