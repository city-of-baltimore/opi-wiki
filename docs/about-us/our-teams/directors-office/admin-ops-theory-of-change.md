# AdminOps Theory of Change

{{ page_header(summary="How AdminOps moves from inputs to outcomes.", category="SERVICE THEORY OF CHANGE") }}

**AdminOps**

*The operating backbone that keeps OPI’s work visible, coordinated, and sustained.*

## NORTH STAR

**A small office that runs on routines, not heroics.**

The AdminOps service keeps OPI’s work coordinated, well-documented, properly resourced, and clearly communicated to City leadership, agencies, partners, and the public. It is the operating backbone that lets the other four services do their jobs.

> *Clarity is not bureaucracy. Clarity is how we protect people, quality, and impact.*


### 1. Service overview

| **Topic**               | **Summary**                                                                                                                                                                                                                            |
|-------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| What it is              | The administrative, operational, communications, and portfolio management functions that support the Mayor’s Office of Performance and Innovation.                                                                                     |
| Why it matters          | A small, multidisciplinary office cannot deliver consistently without strong intake, prioritization, documentation, communications, and quality assurance. AdminOps is the discipline that makes the rest of OPI’s work hold together. |
| Who we serve            | OPI team leads and staff; agencies engaging OPI; CA, Deputy Mayors, and DCAs; Council; partners; public audiences for OPI’s narrative and artifacts.                                                                              |
| Portfolio home          | Director’s Office. Led by the Chief of Staff.                                                                                                                                                                                          |
| What success looks like | Intake is fast and predictable; portfolio status is current; communications and narrative are coordinated; QA prevents low-quality artifacts from going public; OPI’s knowledge persists across staffing changes.                      |

### 2. Operating scope (boundary lines)

#### In scope

- Portfolio intake and tracking — single intake path, prioritization, routing, status visibility.

- Project management coordination — cross-team dependencies, calendaring, milestone tracking.

- Executive briefings — briefing books, talking points, prep for the Mayor, CA, Deputy Mayors, and DCAs.

- Communications, branding, and narrative — public summaries, website updates, "What we heard / what we changed" content, social and earned media.

- Council relations — testimony, reports, hearing prep, and proactive briefings.

- Knowledge management — system of record, document standards, wiki, templates, glossary, onboarding materials.

- Fiscal and procurement coordination — budget, grants, MOUs, procurements that support OPI’s portfolios.

- HR and facilities support — hiring, onboarding, offboarding, performance standards, telework, space and equipment.

- Internal operating routines — leadership norms, standing meetings, retrospectives, all-hands.

- Quality assurance for public-facing materials — QA standards, publication readiness reviews, brand consistency.

- Field validation support — coordination with portfolios on ride-alongs, inspections, and ground-truth checks.

#### Out of scope (we partner or refer)

- Citywide HR, IT, finance, and procurement systems — owned by their respective departments. We coordinate; we do not replace.

- Agency-specific operations — owned by agencies.

- Stat methodology, data engineering, product builds, or service redesigns — owned by the other four services.

### 3. Engagement model

AdminOps is mostly continuous and rhythm-driven, not project-based. Where projects do occur (e.g., a website refresh or a new operating routine), they use the standard OPI engagement model.

| **Function**                 | **Cadence**                                    | **Standard outputs**                                                          |
|------------------------------|------------------------------------------------|-------------------------------------------------------------------------------|
| Intake desk                  | Continuous; daily triage                       | Intake stub; routing decision; portfolio assignment.                          |
| Portfolio review             | Weekly leadership; monthly all-hands           | Portfolio status snapshot; risks; capacity outlook.                           |
| Communications planning      | Weekly comms standup; monthly narrative review | Editorial calendar; "What we heard / what we changed" updates; press posture. |
| Council and executive prep   | On-demand; standing prep windows               | Briefing books; talking points; Q&A trees.                                    |
| QA and publication readiness | Per artifact                                   | QA checklist; sign-off; brand and accessibility check.                        |
| Knowledge management         | Continuous; quarterly cleanups                 | Updated wiki; glossary; templates; onboarding materials.                      |

### 4. Theory of Change

| **Stage**          | **Concrete, measurable definition**                                                                                                                                                                                                                                        |
|--------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Inputs             | Chief of Staff and AdminOps team; intake system; system of record; brand and design standards; QA standards; communications and partnerships lead; product engineering for OPI internal tools; fiscal, procurement, and HR partners.                                       |
| Activities         | Run intake and routing; maintain portfolio visibility; produce briefings and narrative; coordinate Council and executive engagements; run QA before public release; maintain the knowledge base; staff fiscal and procurement coordination; support hiring and onboarding. |
| Outputs            | Current portfolio status; briefing books; published narrative artifacts; QA-cleared public releases; an up-to-date knowledge base; coordinated hiring and onboarding; clean financials and contracts.                                                                      |
| Outcomes (0–6 mo)  | Intake is faster and more predictable; portfolio status is reliable; QA catches issues before public release; new hires onboard in days, not weeks.                                                                                                                        |
| Outcomes (6–18 mo) | Communications produce a coherent public story across services; Council relations are proactive; the knowledge base reduces repeated questions and supports new hires; OPI’s operating routines run on muscle memory.                                                      |
| Impact (2+ yr)     | OPI is a credible, predictable institution. Leadership can rely on the office. Knowledge persists across staffing changes. The operating model is the product.                                                                                                             |

### 5. Governance and decision rights

| **Role**                                   | **Responsibility**                                                                                                                  |
|--------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| Chief of Staff                             | Owns the AdminOps service. Final decision rights on intake routing, portfolio sequencing, communications posture, and QA standards. |
| Data Storyteller                           | Owns narrative, branding, public-facing artifacts, partner cultivation, and Council coordination in collaboration with the CoS.     |
| CitiStat Inspector (Performance team)      | Field validation, evidence verification, and on-the-ground checks for AdminOps quality and Stat closure.                            |
| Operations Analyst (Innovation Lab team)   | Executive coordination, scheduling, briefings, and follow-through for the Executive Director.                                       |
| Full Stack Engineer                        | Build and maintain OPI’s internal operational tools, intake system, and knowledge base infrastructure.                              |
| Project Manager                            | Cross-portfolio coordination, milestone tracking, and delivery support for AdminOps initiatives.                                    |

### 6. Core offerings and target service levels

| **Offering**                 | **Customer**                       | **When to use**                       | **Target SLA / cadence**                                         |
|------------------------------|------------------------------------|---------------------------------------|------------------------------------------------------------------|
| Intake / portfolio tracking  | Agencies, partners, OPI portfolios | Any new request, problem, or referral | Triage within 3 business days; routing decision within 5.        |
| Comms & narrative            | OPI portfolios; CA; Council        | Public release, briefing, or campaign | Editorial calendar updated weekly; QA before any public release. |
| Council / executive briefing | Mayor, CA, Council                 | Hearing, testimony, briefing window   | Briefing book within 10 business days; talking points within 3.  |
| Knowledge management         | OPI staff                          | New hire, new artifact, new process   | Wiki updated continuously; quarterly cleanup.                    |
| QA / publication readiness   | OPI portfolios                     | Any public-facing artifact            | QA within 3 business days; no public release without sign-off.   |

### 7. Metrics, targets, and learning

- Intake speed — median time from request to routing decision.

- Portfolio currency — share of active initiatives with status updates inside the last week.

- QA effectiveness — defects caught pre-release; public-release defect rate.

- Onboarding speed — time-to-productive for new hires.

- Council and executive readiness — share of briefings delivered on time with full materials.

- Narrative cadence — public releases per quarter; "What we heard / what we changed" coverage.

### 8. Operational handoffs across OPI services

- To Performance — coordination of the Stat calendar, public Stat summaries, and field validation.

- To Citywide Data and Analytics — publishing cadence for data products and dashboards.

- To the Innovation Lab — comms and narrative for products and pilots; partnership cultivation.

- To Cross-Agency Delivery — portfolio visibility, comms, and Council coordination for activations.

#### Appendix · Standard artifact suite

- Intake stub (requestor, problem, due date, agencies, owner, urgency, public risk).

- Portfolio status snapshot (open work, owners, milestones, blockers, risks).

- Briefing book (purpose, audience, key messages, Q&A, supporting data).

- Editorial calendar (drafts, reviews, publish dates, owners).

- QA checklist (brand, accessibility, accuracy, factual citation).

- Onboarding package (welcome, system access, OPI Foundations, mandatory reading, 30-60-90 plan).
