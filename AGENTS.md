...
**Do not delete the local guard as "duplicated by `platform-check`".** That was
attempted and measured. 0.4.0 treats `verify.py --plan ci` as an opaque leaf, so
a `pytest` step added to the `ci` tier of `build_steps()` passes it while the
hosted lane really runs the suite — the same green-but-vacuous failure mode as
the `task --dry` bug. It also misses a missing `timeout-minutes`, an
unallowlisted `run:` command, and an unpinned `uses:` ref. The retirement
condition is in the "Two checkers" note in that module's docstring.
